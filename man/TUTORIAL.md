...
s_file = 'output00000012.xml'  # the snapshot we want to analysze
...
It is easy to figure out which version of pcDataLoader you run.
```python
pc.__version__
```
...
Additionailly, we can retrieve the mesh spacing.\
...
Unlike in the python range function, not only the start value, but also the stop value is inclusive too.
```python
...
The meshgrids are returned as 3 and 4 way tensors numpy array. As shown below, it is easy to subset these tensors.
```python
# all mnp coordinates
...
One to figure out if a particular xyz coordinat is still in side the mesh, another one to translate an xyz coordinate into ijk voxel indices. 
...
+ substrate specific 3D or 2D **meshgrid numpy arrays**.
  To get a 2D meshgrids you can slice though any z stack value, the function will always pick the closest mesh coordinate, the smaller coordinate, if you hit the saddle point between two voxels.
...
```python
# all concentration values at particular coordinate
...
Please note, this dataframes not only hold the exact x,y,z coordinate, and all discrete variables mentioned above,
they list additionally all agent surrounding substrate concentrations, the containing i,j,k voxel coordinate, and the m,n,p mesh center coordinate.
...
Like in the pyMCDs class, for memory consumption and processing speed control, we can specify if we later on want to load microenvironment data and graph data from the snapshots we later on analyze.\
By default, all data related to the snapshot is loaded.\
```python
# fine tuned way of loading a mcds object
...
This list can be manipulated a later be fed into the objects read\_xml function to only read sudden snapshots into memory.\
For example, you want only to analyze the last hour 11, 12, 13 from your run, or data from every other hour will be enough.
```python
...
With a single snapshot you work in exactly same way as with an object loaded by pyMCDS.\
```python
...
pyMCDSts instances provides the similar functionality.\
