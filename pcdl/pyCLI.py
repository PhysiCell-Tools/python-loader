...
######################################
# cell relatd command line functions #
######################################
...
def get_celltype_list():
    ...
    if not s_pathfile.endswith('.xml'):
        s_pathfile = s_pathfile + '/initial.xml'
    if not os.path.exists(s_pathfile):
        sys.exit(f"Error @ pyCLI.get_celltype_list : {args.path} path does not look like a outputnnnnnnnn.xml file or physicell output directory ({args.path}/initial.xml is missing).")
    mcds = pcdl.pyMCDS(
        xmlfile = s_pathfile,
        output_path = '.',
        #custom_data_type,
...
def get_substrate_list():
    ...
    if not s_pathfile.endswith('.xml'):
        s_pathfile = s_pathfile + '/initial.xml'
    if not os.path.exists(s_pathfile):
        sys.exit(f"Error @ pyCLI.get_substrate_list : {args.path} path does not look like a outputnnnnnnnn.xml file or physicell output directory ({args.path}/initial.xml is missing).")
    mcds = pcdl.pyMCDS(
        xmlfile = s_pathfile,
        output_path = '.',
        #custom_data_type,
...
###################################################
# cell agenat and substrate command line function #
###################################################
